...
class NudgeShapesAnchorsVertically(object):
	...
	def __init__(self):
		...
		self.w = vanilla.FloatingWindow(
			(windowWidth, windowHeight), "Nudge Shapes & Anchors Vertically"
		)

		y = padding
		self.w.text1 = vanilla.TextBox((padding, y, -padding, 17),
			"Vertical shift per master (units). Positive = up, negative = down:")
		y += rowHeight + 4
	...
	def shiftLayer(self, layer, shift, allItems):
		...
		if allItems:
			...
			for path in layer.paths:
				for node in path.nodes:
					node.position = (node.position.x, node.position.y + shift)
			# Move all components
			for comp in layer.components:
				comp.position = (comp.position.x, comp.position.y + shift)
			# Move all anchors
			for anchor in layer.anchors:
				anchor.position = (anchor.position.x, anchor.position.y + shift)
		else:
		...
		# Note: unlike horizontal shifts, we do NOT restore layer.width here,
		# since vertical movement doesn't affect advance width.

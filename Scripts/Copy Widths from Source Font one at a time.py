#MenuTitle: Copy Widths from Source Font one at a time
# -*- coding: utf-8 -*-
"""
...
The dialog lets you:
  ...
  • Choose to copy to ALL masters or CURRENT master only
...
class CopyWidthsDialog:
    ...
    def __init__(self):
        ...
        # Get current master
        self.current_master = self.target_font.selectedFontMaster
        self.current_master_index = self.target_masters.index(self.current_master) if self.current_master else 0
    ...
    def _build_window(self):
        ...
        win_h = (
            ...
            + 1 + 12    # divider1
            + rh + 6    # MODE SELECTION label
            + 50 + 12   # radio group for mode selection
            + 1 + 12    # divider between mode and mapping
        ...
        # ── Mode selection (All masters vs Current master only) ──────────────
        self.w.modeSectionLabel = TextBox(
            (p, y, -p, rh),
            "COPY MODE",
            sizeStyle="small",
        )
        ...
        current_master_name = self.target_masters[self.current_master_index].name
        self.w.modeRadio = RadioGroup(
            (p + 10, y, -p, 50),
            [
                "Copy to ALL masters",
                "Copy to CURRENT master only  (%s)" % current_master_name
            ],
            callback=self._mode_changed,
            sizeStyle="regular",
        )
        self.w.modeRadio.set(0)  # Default to "All masters"
        y += 50 + 12

        self.w.dividerMode = HorizontalLine((p, y, -p, 1))
        y += 12
    ...
    def _mode_changed(self, sender):
        """Called when user toggles between All/Current master mode."""
        mode = sender.get()
        # Mode 0 = All masters (enable all popups)
        # Mode 1 = Current master only (disable all except current)
        
        for i, popup in enumerate(self._src_popups):
            if mode == 0:
                # All masters mode - enable all popups if source is loaded
                popup.enable(bool(self._src_master_names))
            else:
                # Current master only mode - enable only current master's popup
                popup.enable(i == self.current_master_index and bool(self._src_master_names))
    ...
    def _refresh_mapping_ui(self):
        ...
        mode = self.w.modeRadio.get()
        
        for i, popup in enumerate(self._src_popups):
            popup.setItems(items)
            # Enable based on mode: 0=all, 1=current only
            if mode == 0:
                popup.enable(True)
            else:
                popup.enable(i == self.current_master_index)
    ...
    def _run(self, sender):
        ...
        # Check mode: 0 = all masters, 1 = current master only
        mode = self.w.modeRadio.get()
        copy_all_masters = (mode == 0)
        ...
        for name in self.selected_glyph_names:
            ...
            try:
                if copy_all_masters:
                    ...
                    # Copy to current master only
                    tgt_idx = self.current_master_index
                    master = self.target_masters[tgt_idx]
                    src_idx = master_map[tgt_idx]
                    src_idx = min(src_idx, len(src_widths) - 1)
                    new_width = src_widths[src_idx]
                    layer = glyph.layers[master.id]
        ...
        lines = [
            ...
            "Mode   : %s" % ("All masters" if copy_all_masters else "Current master only (%s)" % self.target_masters[self.current_master_index].name),
            "",
        ]
